...
if __name__ == '__main__':
    ...
    while cap.isOpened():
        ...
        if not ret or frame is None:
            break
        # i += 1
        # if i == 13 or 48 or 93 or 138:
        #     cv2.imwrite(os.path.join(OUT_VISUALIZATIONS_DIR)+ str(i) + '.png', frame)
    ...
    # print(best_period_count.shape)
    # print(best_periodicity_score.shape)
    # Generate video with counts
    rep_frames, rep_frame_num = plots.plot_repetitions(raw_frames[:len(best_period_count)], best_period_count.tolist(), best_periodicity_score.tolist() if not args.no_score else None)
    ...
    for frame in rep_frames:
        video.write(frame)
    video.release()
    print("重复的帧数为：", rep_frame_num, "视频中帧数从1开始。")
    print('Done')
